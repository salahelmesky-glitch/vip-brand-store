...
/* ═══════════════════════════════════════════════════
   ...
   - If email already registered → auto switch to login
   - Once logged in → stays logged in forever (localStorage)
   ═══════════════════════════════════════════════════ */
function AuthForm() {
  ...
  const handle = async (e) => {
    ...
    if (isLogin) {
      ...
      if (!result.success) {
        // If email already exists, auto-switch to login and try login
        if (result.error?.includes('already') || result.error?.includes('مسجل')) {
          const loginResult = await login(email, password);
          if (!loginResult.success) {
            setLocalErr('الإيميل مسجل بالفعل. جرب تسجيل الدخول بالباسورد الصحيح');
            setIsLogin(true);
          }
        } else {
    ...
    <div style={{ maxWidth: 380, width: '100%', margin: '0 auto', padding: '0 16px' }}>
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 800, color: '#f2f2f7', margin: 0, lineHeight: 1.3 }}>
            {isLogin ? 'تسجيل الدخول' : 'حساب جديد'}
          </h2>
          <p style={{ fontSize: 11, color: '#888', marginTop: 4, margin: '4px 0 0' }}>
        ...
        <form onSubmit={handle} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <button type="submit" disabled={loading} style={{
            ...
            boxShadow: '0 4px 16px rgba(191,64,191,0.2)',
          }}>
        ...
        <button
          onClick={() => { setIsLogin(!isLogin); setLocalErr(''); }}
          style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 16, fontWeight: 700, color: '#f2f2f7', margin: 0 }}>
            {user.name}
          </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div style={{
          ...
          <p style={{ fontSize: 20, fontWeight: 800, color: '#f2f2f7', margin: '4px 0 0' }}>
            {user.points}
          </p>
        </div>
        <div style={{
          ...
          <p style={{ fontSize: 20, fontWeight: 800, color: '#00ff66', margin: '4px 0 0' }}>
            {user.tshirtsPurchased || 0}
          </p>
        </div>
      </div>
...
export default function SettingsPage() {
  ...
  return (
    <div style={{
      background: '#050010', minHeight: '100vh', color: '#f2f2f7',
      ...
      <header style={{
        ...
        <span style={{ fontSize: 10, letterSpacing: '0.2em', color: '#bf40bf', fontWeight: 600 }}>
          🔐 ACCOUNT
        </span>
        <div style={{ width: 40 }} />
      </header>
      ...
      <div style={{ paddingTop: 62, paddingBottom: 40, maxWidth: 500, margin: '0 auto', padding: '62px 16px 40px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 30 }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 900, color: '#f2f2f7', margin: '8px 0' }}>
            {isLoggedIn ? 'أهلاً بك! 👋' : 'سجل الدخول'}
          </h1>
